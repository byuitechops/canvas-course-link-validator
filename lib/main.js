...
function main(boundaryDate, courseId) {
    ...
    return course.get()
        ...
        .then((brokenLinksObject) => {
            ...
            var brokenLinksObject = brokenLinksObject;
            if (brokenLinksObject.lastRun <= boundaryDate) {
                console.log('we are going to run again');

                //Run post request to run validator
                return canvas.post(runValidatorLink)
                    .then(() => {
                        var done = false;
                        var i = 0;
                        while (!done && i < 20) {
                            canvas.get(`https://byui.instructure.com/api/v1/courses/${courseId}/link_validation`)
                                .then((linkValidationInfo) => {
                                    done = linkValidationInfo.workflow_state === "completed";
                                    i += 1;
                                    if (!done && i < 20) {
                                        return promise //TODO finsih this line
                                    }
                ...
                return Promise.resolve(brokenLinksObject);
